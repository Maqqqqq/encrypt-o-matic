# twofish_pure.py - pure Python implementation of the Twofish algorithm.
...
class Twofish:
    """Pure Python Twofish (Bjorn Edstrom / Dr Brian Gladman)."""
    ...
    def set_key(self, key):
        ...
        for i in range(key_len // 4):
            key_word32[i] = struct.unpack("<L", key[4 * i:4 * i + 4])[0]
    ...
    def decrypt(self, block):
        ...
        while offset < len(block):
            a, b, c, d = struct.unpack("<4L", block[offset:offset + 16])
            temp = [a, b, c, d]
...
assert Twofish(__testkey).encrypt(__testdat) == b'l\xb4V\x1c@\xbf\n\x97\x05\x93\x1c\xb6\xd4\x08\xe7\xfa'
assert Twofish(__testkey).decrypt(b'l\xb4V\x1c@\xbf\n\x97\x05\x93\x1c\xb6\xd4\x08\xe7\xfa') == __testdat
